...
- **Export/import**: full SQLite backup (.db) and CSV from Settings page. Per-child SQLite export from the journal page dropdown.
...
- **Auto-save**: every change (rating, note, child edit) is written to localStorage immediately and synchronously — no manual save step is required. The nav shows a quiet "✓ Saved" indicator when data exists. The "Backup" button exports a portable .db file; it is not required to persist data within the same browser.
- **No auth**: sign-in and cloud sync have been removed. The app is fully local-first with no authentication.
- **Contact form**: `/contact` submits via a server-side proxy at `/api/contact`. Requires `WEB3FORMS_KEY` (non-VITE_ prefix — kept server-side only, never in the client bundle) set as a Replit secret. The proxy enforces IP-based rate limiting (60 s) and Origin validation in production. If the key is absent the server returns 503 and the form shows a graceful error.
