...
- **Print**: A **Print** button lives on the journal page (`/child/:id`). Clicking it triggers `window.print()` on the journal view. The summary page no longer has a print button or print-only sections.
- **Summary page** (`/child/:id/summary`): Shows overview stats, radar chart, best-fit step by strand (age filter only), area cards, and stagnation alerts. The "Journal pages (print preview)" section and all print-related markup have been removed. The "Include history" toggle defaults to **off**. The best-fit step table no longer shows the Age range column. Nav sections: Overview, Progress, Radar, Best-fit (age filter only), By area, Alerts.
